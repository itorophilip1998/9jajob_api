	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">
				...
				<div class="location">
					<i class="fas fa-map-marker-alt"></i> {{ $detail->listing_address }}
				</div>
				<div class="review">
                    @if($overall_rating == 5)
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyC6yqP8_qWQsmhyqkSrAgTm7CUQ6yHwzRY&callback=initMap" async defer></script>

<script>
	let address_longitude = {{ $detail->address_longitude }};
	let address_latitude = {{ $detail->address_latitude }};	
	let title = "{{ $detail->listing_name }}";

	function initMap() {
		const uluru = { lat: address_latitude, lng: address_longitude };
